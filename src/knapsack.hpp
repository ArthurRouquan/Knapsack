...
/* Returns an iterator of the split item (if none, returns last) and the weight,
profit realized by packing all the previous more efficient items. Runs in O(n)
with n = distance(first, last). Items are expected to be sorted. */
...
/* Returns the LP relaxation's profit in O(k) with k = distance(first, last).
...
/* Returns LP relaxation's profit from the partial sum of items in O(ln k) with
k = end(Σ) - i. Items are expected to be sorted. */
